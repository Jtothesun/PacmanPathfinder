...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    while not fringe.isEmpty():
        ...
        for successor, action, stepcost in problem.getSuccessors(node):
            if (successor not in visited) or (successor in visited and visited[successor] > cost + stepcost + (heuristic(successor, problem))):
                visited[successor] = cost + stepcost
                fringe.push((successor, direction + [action], cost + stepcost), cost + stepcost + (heuristic(successor, problem)))
    return []
    util.raiseNotDefined()
